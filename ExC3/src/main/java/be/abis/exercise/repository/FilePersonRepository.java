...
public class FilePersonRepository implements PersonRepository {
	...
	private String fileLoc = "personsSpring.csv";
	...
	private StringBuilder parsePerson(Person p) {
		...
		Address address = p.getCompany().getAddress();
		if (address != null) {
			int nr = address.getNr();
			...
			.append((p.getAge() != 0 ? p.getAge() : null) + ";").append(p.getEmailAddress() + ";")
			.append(p.getPassword() + ";").append(p.getLanguage().toLowerCase() + ";")
			.append(p.getCompany().getName() + "A;").append("0;")
			.append("0;").append("A;").append("0;").append("A;A");			
		}
